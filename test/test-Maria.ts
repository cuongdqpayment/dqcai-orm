...
class PostService extends BaseService<Post> {
  ...
  // Thay đổi MariaDB placeholders ($1, $2) → MySQL placeholders (?)
  // ==========================================
  ...
  async getPostsWithAuthors(options?: {
    ...
    const adapter: MariaDBAdapter = await this.getAdapter();
  ...
  async getPostsWithTags(options?: {
    ...
    // ✅ FIX 1: Thay MariaDB JSON functions → MySQL JSON functions
    // ✅ FIX 2: Thay $1 → ?
    let query = `
    SELECT 
      p.id,
...
class PostTagService extends BaseService<PostTag> {
  ...
  // MariaDB cho phép GROUP BY chỉ primary key nếu có UNIQUE constraint
  ...
  `;
    ...
    // ✅ Với MariaDB 9.1+, GROUP BY primary key là đủ
...
async function ensureDatabaseExists() {
  const mysql = await import("mariadb");
...
async function main() {
  try {
    console.log("╔═══════════════════════════════════════════════════════╗");
    console.log("║  @dqcai/orm MariaDB Blog Application Demo         ║");
