...
class PostService extends BaseService<Post> {
  ...
  // 🆕 JOIN: Get posts with author information
  async getPostsWithAuthors(options?: {
    limit?: number;
    offset?: number;
  }): Promise<PostWithAuthor[]> {
    const adapter: PostgreSQLAdapter = await this.getAdapter();
    const query = `
      SELECT 
        p.*,
  ...
  // 🆕 JOIN: Get single post with author
...
class CommentService extends BaseService<Comment> {
  ...
  // 🆕 JOIN: Get comments with post and user details
...
class PostTagService extends BaseService<PostTag> {
  ...
  // 🔧 FIXED VERSION - Thay thế method getPostsWithTags() trong PostTagService
  ...
  async getPostsWithTags(options?: {
    ...
    // ✅ FIX: Thêm TẤT CẢ các cột của posts vào GROUP BY
    let query = `
    SELECT 
      p.id,
      p.title,
  ...
  `;
    ...
    // ✅ FIX: GROUP BY tất cả các cột được SELECT (trừ aggregate function)
    query += ` 
    GROUP BY 
      p.id, 
      p.title, 
  ...
  // PostgreSQL cho phép GROUP BY chỉ primary key nếu có UNIQUE constraint
  ...
  `;
    ...
    // ✅ Với PostgreSQL 9.1+, GROUP BY primary key là đủ
...
async function ensureDatabaseExists() {
  ...
  // Kết nối vào database 'postgres' (database mặc định)
  ...
  try {
    ...
    if (result.rows.length === 0) {
      ...
      await client.query("CREATE DATABASE blog_app");
      console.log('✓ Database "blog_app" created');
    } else {
      console.log('✓ Database "blog_app" already exists');
    }
  } catch (error) {
...
async function example8_ComplexJoins() {
  ...
  // 1. Get user activity summary (posts + comments count)
  ...
  const mostCommented = await adapter.raw(mostCommentedQuery, ["published", 5]);
  ...
  // 3. Get popular tags with recent posts
  ...
  popularTags.rows.forEach((tag: any) => {
    console.log(`  - ${tag.name} (${tag.post_count} posts)`);
    if (tag.recent_posts) {
      const recentPosts = tag.recent_posts.slice(0, 2);
  ...
  // 4. Get comment thread with nested replies
  ...
  const commentThreadQuery = `
    WITH RECURSIVE comment_tree AS (
      ...
      INNER JOIN users u ON c.user_id = u.id
      INNER JOIN posts p ON c.post_id = p.id
      WHERE c.parent_id IS NULL AND c.is_approved = true
    ...
    ORDER BY post_title, parent_id NULLS FIRST, id
    LIMIT $1
  `;

  const commentThreads = await adapter.raw(commentThreadQuery, [10]);
  let currentPost = "";
...
async function example10_BulkOperations() {
  ...
  const adapter = DatabaseManager.getAdapterInstance("blog_app");
  if (adapter) {
    await adapter.raw(`UPDATE users SET is_active = $1 WHERE is_active = $2`, [
      true,
      false,
    ]);
...
async function main() {
  try {
    console.log("╔═══════════════════════════════════════════════════════╗");
    console.log("║  @dqcai/orm PostgreSQL Blog Application Demo         ║");
