// ./test/test-enhanced-sqlite.ts
// Enhanced test script for core, oms, and payment databases
import {
  ServiceManager,
  BaseService,
  DatabaseManager,
...
class OrderItemService extends BaseService {
  ...
  async getOrderTotal(orderId: string) {
    ...
    return items.reduce((sum: number, item: any) => sum + parseFloat(item.total_price), 0);
  }
}
...
async function verifyForeignKeys(schemaName: string, tables: string[]) {
  ...
  const adapter = DatabaseManager.getAdapterInstance(schemaName) as SQLiteAdapter;
...
async function main() {
  try {
    ...
    console.log("✅ Dine-in transaction created:", dineInTransaction?.transaction_number);
    ...
    const confirmedOrders = await orderService.findByStatus(storeId, "confirmed");
    console.log(`✅ Confirmed orders: ${confirmedOrders.length}`);

    // Query transactions
    const completedTransactions = await transactionService.findByStatus(storeId, "completed");
    console.log(`✅ Completed transactions: ${completedTransactions.length}`);

    const pendingTransactions = await transactionService.findByStatus(storeId, "pending");
    console.log(`✅ Pending transactions: ${pendingTransactions.length}`);

    // Query payment transactions
    const successfulPayments = await paymentTransactionService.getSuccessfulTransactions(storeId);
    console.log(`✅ Successful payments: ${successfulPayments.length}`);

    // Query online orders
    const pendingDeliveries = await onlineOrderService.getPendingDeliveries(storeId);
    ...
    await serviceManager.executeSchemaTransaction("core", async (coreServices) => {
